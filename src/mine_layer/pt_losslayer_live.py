...
class MyLossLayer(caffe.Layer):
    """Controallable list-wise ranking function."""
    ...
    def forward(self, bottom, top):
        ...
        for k in range(dis):
            ...
            self.margin = (100-bottom[1].data[k*SepSize]) / level
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        for k in range(dis):
		if (bottom[0].data[(k+1)*SepSize-1] > 100):
			self.ref[(k+1)*SepSize-1] += (bottom[0].data[(k+1)*SepSize-1]-100)*bottom[0].num/100
